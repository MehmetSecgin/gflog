...
fn main() {
    let cli = Cli::parse();
    match cli.source {
        Source::File { file, json, filter, view } => {
            ...
            if recs.len() == total {
                ...
                eprintln!("# {name}  ({} of {total} records after filter)\n", recs.len());
            }
            views::dispatch(&recs, &view, json);
        }
        Source::Live { query, limit, json, time, conn, ns, filter, view } => {
            live::run_live(&query, &ns, limit, json, &time, &conn, &filter, &view);
        }
        Source::Metric { query, step, json, time, conn, ns } => {
            live::run_metric(&query, &ns, &step, json, &time, &conn);
        }
        Source::Labels { json, time, conn, ns } => live::run_labels(&ns, json, &time, &conn),
        Source::Values { label, query, json, time, conn, ns } => {
            live::run_values(&label, query.as_deref(), &ns, json, &time, &conn);
        }
        Source::Cookie { value, stdin, test } => live::cmd_cookie(value, stdin, test),
        Source::Token { value, stdin, test, clear } => live::cmd_token(value, stdin, test, clear),
